...
pub fn get_file_type(filename: &Path) -> Result<String> {
    ...
    if config::EXT_DOCUMENT.contains(&ext.as_str()) {
        Ok("word".to_string())
    } else if config::EXT_PRESENTATION.contains(&ext.as_str()) {
        Ok("slide".to_string())
    } else if config::EXT_SPREADSHEET.contains(&ext.as_str()) {
        Ok("cell".to_string())
    } else {
        Err(Error::FileUtils)
    }
