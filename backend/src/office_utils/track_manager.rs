    ...
    if cur_ext != download_ext {
        ...
        match new_uri {
            ...
            Err(e) => {
                let f = format!(
                    "{}{}",
                    file_utils::get_file_name_no_ext(std::path::Path::new(filename))?,
                    download_ext
                );
...
pub async fn process_force_save(body: CallbackData, filename: &str, user_id: i32) -> Result<()> {
    ...
    if is_submit_form {
        if new_filename {
            filename = doc_manager::get_correct_name(&format!(
                "{}-form{}",
                file_utils::get_file_name_no_ext(std::path::Path::new(&filename))?,
                filetype
            ))?
        ...
        forcesave_path = doc_manager::get_forcesave_path(&filename, user_id, true)
            .await
            .unwrap();
    } else {
        if new_filename {
        ...
        forcesave_path = doc_manager::get_forcesave_path(&filename, user_id, false)
            .await
            .unwrap();
        if forcesave_path.to_str().unwrap().is_empty() {
            forcesave_path = doc_manager::get_forcesave_path(&filename, user_id, true)
                .await
                .unwrap();
        }
