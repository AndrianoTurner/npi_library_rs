...
use crate::config::{
    DOCUMENT_SERVER_URL, DOC_SERV_API_URL, DOC_SERV_CONVERT, DOC_SERV_EDITED, DOC_SERV_FILLFORMS,
    DOC_SERV_VIEWED, ROOT_FOLDER, STATIC_URL,
};
...
pub fn is_supported_extension(file_extension: &str) -> bool {
    is_can_fill_forms(file_extension)
        || is_can_view(file_extension)
        || is_can_edit(file_extension)
        || is_can_convert(file_extension)
}
...
pub fn get_internal_extension(file_type: &str) -> String {
    ...
    mapping
        .get(file_type)
        .map_or("docx".to_owned(), |f| f.to_string())
}
...
pub fn get_template_image_url(filetype: &str) -> String {
    ...
    mapping
        .get(filetype)
        .map_or("file_docx.svg".to_string(), |f| f.to_string())
}
...
pub async fn create_sample(filetype: &str, is_sample: bool, user_id: i32) -> Result<String> {
    ...
    let mut asset_file = tokio::fs::File::open(format!("assets/{}/{}", sample_name, filename))
        .await
        .unwrap();
